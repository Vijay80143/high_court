...
interface SidebarProps {
  activeTab: 'dashboard' | 'display' | 'tracker' | 'settings';
  setActiveTab: (tab: 'dashboard' | 'display' | 'tracker' | 'settings') => void;
}
...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  const menuItems = [
    ...
    { id: 'display', label: 'Display Boards', icon: (
    ...
    { id: 'settings', label: 'Firm Settings', icon: (
    ...
    <aside className="w-64 bg-slate-900 h-screen flex flex-col text-slate-300">
      <div className="p-6 border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-white text-lg tracking-tight">LegalSync Pro</span>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-slate-800">
        <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700/50">
          <p className="text-[10px] uppercase tracking-wider text-slate-500 font-bold mb-2">Active Subscription</p>
          <p className="text-xs text-slate-200 font-medium">Enterprise Plan</p>
